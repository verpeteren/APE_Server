...
#endif
#ifdef _USE_POSTGRESQL
static JSClass postgresql_class = {
	"PostgreSQL", JSCLASS_HAS_PRIVATE,
		JS_PropertyStub, JS_PropertyStub, JS_PropertyStub, JS_StrictPropertyStub,
		JS_EnumerateStub, JS_ResolveStub, JS_ConvertStub, apepostgresql_finalize,
		JSCLASS_NO_OPTIONAL_MEMBERS
};
#endif
...
{	
	...
	if (queue->nParams == 0) {
		...
		rc = PQsendQueryParams(myhandle->conn, queue->statement, queue->nParams, NULL, queue->paramValues, queue->paramLengths, NULL, 1);
	}
	if (rc != 0 ) {
...
static struct _ape_postgresql_queue *apepostgresql_push_queue(struct _ape_postgresql_data *myhandle, char * statement, const int *paramLength, const char *const *paramValues, int nParams, jsval callback)
{
 ...
 *
 *
 * @name Ape.PostgreSQL.errorString
 * @function
 * @public
 *
 * @returns {string} the last error message for this connection
 *
 * @example
 * Ape.log(sql.onError());
 */
...
//{
	...
	JS_SET_RVAL(cx, vpn, STRING_TO_JSVAL(JS_NewStringCopyZ(cx, PQerrorMessage(myhandle->conn))));
...
//{
	...
	int i;
	JSObject *obj = JS_THIS_OBJECT(cx, vpn);
	const char *keywords[] = {"server_version", "server_encoding", "client_encoding", "is_superuser", "session_authorization", "DateStyle", "TimeZone", "integer_datetimes", "server_version", "server_encoding", "client_encoding"};
	if ((myhandle = JS_GetPrivate(cx, obj)) == NULL) {
		return JS_TRUE;
	}
	JSObject *statusObj = JS_NewObject(cx, NULL, NULL, NULL);
	JS_AddObjectRoot(cx, &statusObj);
	char *cvalue;
	char *ckeyword;
#if 0 //for v9.3
	//get the data from PQconninfo
	PQconninfoOption *option = PQconninfoOption(myhandle->conn);
	while (option != NULL) {
		ckeyword = xstrdup(option->keyword);
		if(option->val == NULL){
			JS_SetProperty(cx, statusObj, JSVAL_NULL);
		} else {
			cvalue = xstrdup(option->val);
			value = STRING_TO_JSVAL(JS_NewStringCopyZ(cx, cvalue));
			JS_SetProperty(cx, statusObj, ckeyword, &value);
		free(ckeyword);
		free(cvalue);
	}
#else
	cvalue = PQhost(myhandle->conn);
	if (cvalue == NULL) {
		value = JSVAL_NULL;
	} else {
		value = STRING_TO_JSVAL(JS_NewStringCopyZ(cx, cvalue));
	...
	JS_SetProperty(cx, statusObj, "host", &value);
	cvalue = PQdb(myhandle->conn);
	if (cvalue == NULL) {
		value = JSVAL_NULL;
	} else {
		value = STRING_TO_JSVAL(JS_NewStringCopyZ(cx, cvalue));
	...
	JS_SetProperty(cx, statusObj, "dbname", &value);
	cvalue = PQuser(myhandle->conn);
	if (cvalue == NULL) {
		value = JSVAL_NULL;
	} else {
		value = STRING_TO_JSVAL(JS_NewStringCopyZ(cx, cvalue));
	...
	JS_SetProperty(cx, statusObj, "user", &value);
	cvalue = PQport(myhandle->conn);
	if (cvalue == NULL) {
		value = JSVAL_NULL;
	} else {
		value = INT_TO_JSVAL(atoi(cvalue));
		free(cvalue);
	}
	JS_SetProperty(cx, statusObj, "port", &value);
	i = PQprotocolVersion(myhandle->conn);
	value = INT_TO_JSVAL(i);
	JS_SetProperty(cx, statusObj, "protocol_version", &value);
	i = PQsocket(myhandle->conn);
	value = INT_TO_JSVAL(i);
	JS_SetProperty(cx, statusObj, "socket_version", &value);
	i = PQbackendPID(myhandle->conn);
	value = INT_TO_JSVAL(i);
	JS_SetProperty(cx, statusObj, "backend_pid", &value);
	i = PQconnectionNeedsPassword(myhandle->conn);
	value = (i == 1) ? JSVAL_TRUE: JSVAL_FALSE;
	JS_SetProperty(cx, statusObj, "connection_needs_password", &value);
	...
	for (i = 0; i < sizeof(keywords)/sizeof(keywords[0])  ; i++) {
		ckeyword = xstrdup(keywords[i]);
		const char *tvalue = PQparameterStatus(myhandle->conn, ckeyword);
		if (tvalue == NULL) {
			value = JSVAL_NULL;
			JS_SetProperty(cx, statusObj, ckeyword, &value);
		} else {
			cvalue = xstrdup(tvalue);
			value = STRING_TO_JSVAL(JS_NewStringCopyZ(cx, cvalue));
			JS_SetProperty(cx, statusObj, ckeyword, &value);
		free(ckeyword);
		free(cvalue);
	...
	JS_SET_RVAL(cx, vpn, OBJECT_TO_JSVAL(statusObj));
...
if (JSVAL_IS_NULL(jsval)) {
	...
	JS_free(cx, cconninfo);
		...
		const char allowedKeys[][26] = {"host", "hostaddr", "port", "dbname", "user", "password", "connect_timeout", "client_encoding", "options", "application_name", "fallback_application_name", "keepalives", "keepalives_idle", "keepalives_interval", "keepalives_count", "tty", "sslmode", "requiressl", "sslcompression", "sslcert", "sslkey", "sslrootcert", "sslcrl", "requirepeer", "krbsrvname", "gsslib", "service"};
		const char *keywords[27] = {NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};
	...
	myhandle->queue.head = NULL;
	myhandle->queue.foot = NULL;
	//JS_AddObjectRoot(myhandle->cx, &obj);
	JS_SetPrivate(cx, obj, myhandle);
	JS_DefineFunctions(cx, obj, apepostgresql_funcs);
	//JS_AddValueRoot(myhandle->cx, &rval);
	JS_SET_RVAL(cx, vpn, OBJECT_TO_JSVAL(obj));
	if (statusType == CONNECTION_BAD || fd == -1 || status == PGRES_POLLING_FAILED) {
		params[0] = INT_TO_JSVAL(-1);
		JS_CallFunctionName(myhandle->cx, myhandle->jspostgresql, "onError", 1, params, &rval);
	} else {
		prepare_ape_socket (fd, g_ape);
		events_add(g_ape->events, fd, EVENT_READ|EVENT_WRITE);
		g_ape->co[fd]->fd = fd;
		...
		g_ape->co[fd]->data = myhandle;
		params[0] = INT_TO_JSVAL(0);
		JS_CallFunctionName(cx, obj, "onConnect",  0, NULL, &rval);
		myhandle->state = SQL_READY_FOR_QUERY;
	}
	return JS_TRUE;
	/**
...
static void ape_sm_define_ape(ape_sm_compiled *asc, JSContext *gcx, acetables *g_ape)
{
	...
	#endif
	#ifdef _USE_POSTGRESQL
	JS_DefineFunctions(asc->cx, jspostgresql, apepostgresql_funcs);
	#endif
